...
class PictureminatorApp(ctk.CTk):
    def __init__(self):
        ...
        self.center_frame.place(relx=0.5, rely=0.65, anchor='center')

        # Create a new frame for the search bar and checkbox
        self.search_frame = ctk.CTkFrame(self.center_frame)
        self.search_frame.pack(pady=10, fill='x') #padx=50, 

        # Place the entry and button in the search frame instead of center frame
        self.path_entry = ctk.CTkEntry(self.search_frame, placeholder_text="Select folder...", width=500)
        self.path_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))  # Add padding to separate entry and button
        ...
        try:
            ...
            # Create the button with the correct image object and pack it inside search_frame
            self.browse_button = ctk.CTkButton(self.search_frame, text=" ", image=self.search_icon, command=self.select_folder)
            self.browse_button.image = self.search_icon  # keep a reference!
            self.browse_button.pack(side="left", padx=(0, 10))  # Adjust side to "left" and pack inside search_frame
        except IOError as e:
    ...
    def select_folder(self):
        ...
        print("Folder select action triggered")
